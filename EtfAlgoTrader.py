...
import win32com.client #사실상 win32com이 연결을 맡으면서 모든 역할을 다 하고 있는데?
import pandas as pd
...
# 정적 html정도만 있었다면 beautifulSoup만 사용해서 태그랑 id정도만 읽어왔을 텐데 실시간으로 변하는 가격을 반영하는 동적페이지라 selenium을 사용
from selenium.webdriver.chrome.options import Options 
from slack_sdk import WebClient
from slack_sdk.errors import SlackApiError

slack = WebClient(token='[my_slack_api_token]')

def dbgout(message):
    """인자로 받은 문자열을 파이썬 셸과 슬랙으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message
    slack.chat_postMessage( channel = '#notgeneral', text = strbuf)
...
def check_creon_system():
    ...
    # 연결 여부 체크
    if (cpStatus.IsConnect == 0): # 이 구문때문에 creonplus를 열어주는 소스코드는 또 따로 필요한 거구
...
def buy_etf(code):
    ...
    try:
        ...
        print('ask_price 존재여부 점검 ',ask_price)
        if ask_price > 0:
            buy_qty = buy_amount // ask_price
        buy_qty = int(buy_qty)
        stock_name, stock_qty = get_stock_balance(code)  # 종목명과 보유수량 조회
        print('soctk_name = ', stock_name ,  '내가 가진 해당 etf의 수(stock_qty) = ', stock_qty)
        ...
        if current_price > target_price and current_price > ma5_price \
            ...
            # 0: 정상,  그 외 오류, 4: 주문요청제한 개수 초과 -->  처음에는 계속 ret=5였는데 아마 너가 특정 etf에 대해 주문한 개수는 0개인데 그러면 왜 주문 넣은거야? 이런의미인
            ...
            if bought_qty > 0:
                ...
                dbgout('현재 bought_list' , bought_list)
        print('stockname과 stockqty 출력이후 바로 이 문장이라면 가격조건이 맞지 않아 바로 다음 sym으로 넘어가는 겁니다' ,'\n\n\n')
    except Exception as ex:
...
if __name__ == '__main__': 
    try:
        symbol_list = ['A360750 ', 'A252670', 'A233740', 'A250780', 'A225130',
             'A280940', 'A261220', 'A217770', 'A295000', 'A176950']
                # 여기가 앞에서 배운 동적 스크래핑으로 거래 가능한 etf들을 읽어서 쫙 돌릴 수 있는 부분
        bought_list = []     # 매수 완료된 종목 리스트
        target_buy_count = 1 # 매수할 종목 수 --> 일단 1개만 으로 테스트부터 성공시키
        buy_percent = 0.95   
        ...
        while True: # 이것때문에 계속 멘트가 나오는구나 -> 어쨋든 계속 돌리라는 말이니까
            print('while True에 의해 다시 루프의 시작')
            t_now = datetime.now()
            ...
            if t_start < t_now < t_sell :  # AM 09:05 ~ PM 03:15 : 매수
                ...
                for sym in symbol_list:
                    if len(bought_list) < target_buy_count:
                        print('이번에는', sym , '의 buy_etf 수행 차례입니다')
                        buy_etf(sym)
                        time.sleep(1)
                if t_now.minute == 30 and 0 <= t_now.second <= 5:
                    print('t_now.minute == 30 and 0 <= t_now.second <= 5라서 get_stock_balance("ALL")시작')
